...
export const signExpiringResourceURLs = (_rootUrl: string, pathBase64Str: string, jsonObj: any): string | undefined => {
    ...
    checkSumData.update(`${timestamp}_${hashSecret}_${expiry}_${pathBase64Str}`);
    ...
    traverseJsonObjects(jsonObj,
        (obj) => {
            if (obj.href && typeof obj.href === "string"
                ...
                const fakeRootUrl = "https://dummy.com/";
                const url = new URL(obj.href, fakeRootUrl); // `${rootUrl}/manifest.json`
                // implicit encodeURIComponent_RFC3986()
                url.searchParams.append(URL_SIGNED_EXPIRY_QUERY_PARAM_NAME, queryParamValue);
                obj.href = url.toString().replace(fakeRootUrl, "");
                // debug(`------------ Link HREF: ${obj.href}`);
            }
        });
...
export const verifyExpiringResourceURL = (queryParamValue: string | undefined, pathBase64Str: string, _pathInZip: string): boolean => {
    ...
    try {
        ...
        const checkSumData = crypto.createHash("sha256");
        checkSumData.update(`${json.timestamp}_${hashSecret}_${json.expiry}_${pathBase64Str}`);
        const hashData = checkSumData.digest("hex");
        if (hashData !== json.hash) {
            debug(`HASH diff! ${hashData} !== ${json.hash})`);
            return false;
        }
        ...
        if (timeDiff > (json.expiry * 1000)) {
            debug(`Resource EXPIRED! ${timeDiff} > ${json.expiry})`);
            return false;
        }
