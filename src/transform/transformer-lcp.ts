...
export class TransformerLCP implements ITransformer {
    ...
    public supports(publication: Publication, link: Link): boolean {
        ...
        const check = publication.LCP.Encryption.Profile === "http://readium.org/lcp/basic-profile"
        ...
        const passPhrase = publication.LCP.getUserPassphraseHex();
        if (!passPhrase) {
            debug("Missing LCP user passphrase.");
            return false;
        }
        ...
        const userKey = new Buffer(passPhrase, "hex");

        const keyCheck = new Buffer(publication.LCP.Encryption.UserKey.KeyCheck, "base64");
        // .toString("binary");
        ...
        if (publication.LCP.ID !== decryptedOut) {
            debug("Failed LCP ID check.");
            return false;
        }

        const encryptedContentKey =
            new Buffer(publication.LCP.Encryption.ContentKey.EncryptedValue, "base64");
        // .toString("binary");
        ...
        publication.LCP.ContentKey = decrypted2.slice(0, size2); // .toString("binary");
        ...
        if (!publication.LCP.ContentKey) {
            debug("Cannot get LCP content key.");
            return false;
        }
